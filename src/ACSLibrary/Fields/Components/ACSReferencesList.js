...
function ACSReferencesList(props)  {
  const {object_type, mode, reference_formValues, reference_lastTouched , referenced_object_type, referenced_id, field_name, field_list, num_add=3, allow_add=true, allow_save=false,...params} = props


  if (["edit","create"].includes(mode)) {

    return (<ACSListController {...params} field_list={field_list} no_header={true} list_pagination={false} list_mode="list_edit" row_form={true} field_click_to_edit={false} num_add={num_add} allow_add={allow_add} allow_save={allow_save} object_type={object_type} api_options={props.api_options} referenced_object_type={referenced_object_type} referenced_id={referenced_id} reference_formValues= {reference_formValues} reference_lastTouched = {reference_lastTouched} reference_field_name={field_name}
/> )
  } else {
