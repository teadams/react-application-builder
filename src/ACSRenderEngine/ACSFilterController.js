...
function ACSFilterController(props) {
  const {filters={}, default_filter_values={}, onChange, label_direction="column", label_variant="subtitle1", filter_direction="column", label_width="70px", select_width="250px", select_display_field, label_position="select"} = props
    ...
    //    u.a(filter.name, filter.select_api_options)
        if (filter.filter_type !== "Text") {
            if (!filter.select_display_field) {
              ...
              if (label_position==="selects") {
                filter.option_label = filter.label
                filter.label = ""
              }
            }
        }
        return (
          ...
          <div style={{alignSelf:"flex-start", minWidth:select_width}}>
            ...
            <ACSSelectFilter label={filter.option_label} key={filter.name} object_type={filter.object_type} filter_name={filter.name} field_name={filter.name} default_value={formValues[filter.name]} onChange={handleFilterChange} api_options={filter.select_api_options} select_display_field={filter.select_display_field}/>}
