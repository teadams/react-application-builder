...
function RenderACSRow(props) {
  const { ...params} = props
  const {key_id, form, form_title, mode, form_open, data, rab_component_model, object_type, field_list, chunked_field_list, sections=[], handleFormChange, handleFormSubmit, formAttributes, onClose, header_image_size="small", field_models, object_models,  num_columns, dialog_size, dialog_center, delayed_auth=false, form_submit_word, close_action} = props
  ...
  if (data) {
    return (
      <Auth require_authorization={!delayed_auth} auth_action={mode} object_type={object_type} onClose={onClose} data={data} object_models={object_models}>
