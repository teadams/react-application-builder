...
function RenderACSRow(props) {
  const { ...params} = props
  const {key_id, form, form_title, mode, form_open, data, rab_component_model, object_type, field_list, chunked_field_list, sections=[], handleFormChange, handleFormSubmit, formAttributes, onClose, header_image_size="small", num_columns, dialog_size, dialog_center, delayed_auth=false} = props
  ...
  if (data) {
    return (
      <Auth require_authorization={!delayed_auth} auth_action={mode} object_type={object_type} onClose={onClose} data={data}>
        <FormWrap mode={mode} form={form} object_type = {object_type} open={form_open} dialog_size={dialog_size} form_title={form_title} dialog_center={dialog_center} onSubmit={handleFormSubmit} onClose={onClose} key={"row_form"}  >
         <HeaderWrap {...params} >
